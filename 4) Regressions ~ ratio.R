...
# RP-only ratio model complete list of independent variables for Models
rp_indep_ratio <- c("ONES", "LN_RATIO", "GOV", "COMP", "ONPV", "VTYPE_TRUCK", "VTYPE_SUV", "VTYPE_VAN", "BADGE", "FEMALE", "LN_AGE", "LN_TMPY", "ENV_ETH", "ENV_GAS", "ENV_ND","ENG_ETH", "ENG_GAS", "ENG_ND", "ECON_ETH", "ECON_GAS", "ECON_ND", "NS_ETH", "NS_GAS", "NS_ND","MPG_ETH", "MPG_GAS", "MPG_ND",  "STNST_AR", "STNST_CO",  "STNST_OK", "STNST_CA")

# SP-off-RP ratio model list of independent variables for Models
sp_indep_ratio <- c("ONES", "LN_HRATIO", "GOV", "COMP", "ONPV", "VTYPE_TRUCK", "VTYPE_SUV", "VTYPE_VAN", "BADGE", "FEMALE", "LN_AGE", "LN_TMPY", "ENV_ETH", "ENV_GAS", "ENV_ND", "ENG_ETH", "ENG_GAS", "ENG_ND", "ECON_ETH", "ECON_GAS", "ECON_ND", "NS_ETH", "NS_GAS", "NS_ND", "MPG_ETH", "MPG_GAS", "MPG_ND",  "STNST_AR", "STNST_CO",  "STNST_OK", "STNST_CA")
...
dta <- dta[,unique(c(rp_dep, sp_dep, rp_indep_ratio, sp_indep_ratio, svy_wts, "QD", "DIST", "RATIO"))]
rownames(dta) <- NULL
...
r2 <- cbind(sum_func_lnratio_new("A"), sum_func_lnratio_new("B"), sum_func_lnratio_new("C")) %>% round(3)

if(nrow(dta) == 479){write.xlsx(r2, file="Results/Table 6 - 479 obs.xlsx")}
if(nrow(dta) > 479){write.xlsx(r2, file="Results/Table 6 - 670 obs.xlsx")}
...
if(nrow(dta) == 479){write.xlsx(ratio_mfx, file= "Results/Table 4.xlsx")}
if(nrow(dta) > 479){write.xlsx(ratio_mfx, file= "Results/Table 5.xlsx")}

########################
###   Save Results   ###
########################

if(nrow(dta) == 479){save.image("Results/SP-off-RP with 479 obs - ratio.RData")}
if(nrow(dta) > 479){save.image("Results/SP-off-RP with 670 obs - ratio.RData")}
