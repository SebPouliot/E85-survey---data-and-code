...
######################################
### Ratio  - with 670 observations ###
######################################

load("Results/SP-off-RP with 670 obs - ratio.RData")

rm(list = objects()[!(objects() %in% c("dta", "modA", "modB", "rp_indep_ratio"))])
...
colnames(dta_graph) <- c("ratio", "prob_RP_Y", "se_RP_Y", "prob_RP_Z", "se_RP_Z", "prob_SPRP_Y", "se_SPRP_Y", "prob_SPRP_Z", "se_SPRP_Z")
dta_graph <- dta_graph %>% mutate(ratio = seq(0.5, 1.25, length.out = nrow(dta_graph)))

source("Code/Demand graph func.R")

for (i in 1:nrow(dta_graph)){
  probs_RP <-  prob_e85_lnratio(coef_est = mod_RP_ratio$coef, vcov_est = mod_RP_ratio$vcov, data = dta, lnratio = log(dta_graph$ratio[i]))
  ...
  probs_SPRP <-  prob_e85_lnratio(coef_est = setNames(as.numeric(mod_SPRP_ratio$coef[rp_indep_ratio,1]), rp_indep_ratio), vcov_est = mod_SPRP_ratio$vcov[-nrow(mod_SPRP_ratio$vcov),-ncol(mod_SPRP_ratio$vcov)], data = dta, lnratio = log(dta_graph$ratio[i]))
...
dta_mean <- dta_graph %>% 
    dplyr::select(ratio, prob_RP_Y, prob_SPRP_Y, prob_RP_Z, prob_SPRP_Z) %>% 
    gather(key, value, -ratio) %>% 
    mutate(retailer = ifelse(str_detect(key, "_Y"), "Retailer Y", "Retailer Z"), 
           key = ifelse(str_detect(key, "_RP"), "A) RP data", "B) SP-off-RP"))
  
dta_min <- dta_graph %>% 
    dplyr::select(ratio, min_RP_Y, min_SPRP_Y, min_RP_Z, min_SPRP_Z) %>% 
    gather(key, min, -ratio) %>% 
    mutate(retailer = ifelse(str_detect(key, "_Y"), "Retailer Y", "Retailer Z"), 
           key = ifelse(str_detect(key, "_RP"), "A) RP data", "B) SP-off-RP"), 
           min = ifelse(min<0, 0, min))
  
dta_max <- dta_graph %>% 
    dplyr::select(ratio, max_RP_Y, max_SPRP_Y, max_RP_Z, max_SPRP_Z) %>% 
    gather(key, max, -ratio) %>% 
    mutate(retailer = ifelse(str_detect(key, "_Y"), "Retailer Y", "Retailer Z"), 
           key = ifelse(str_detect(key, "_RP"), "A) RP data", "B) SP-off-RP"), 
...
load("Results/SP-off-RP with 479 obs - ratio.RData")

rm(list = objects()[!(objects() %in% c("dta", "modA", "modB", "rp_indep_ratio", "dta_graph_670"))])
...
plot <- ggplot(dta_graph, aes(y = value, x = ratio, color = key, fill = key, ymin = min, ymax = max)) + 
  geom_ribbon(alpha = 0.5, linetype = 1, color = "black", size = 0.25) + 
  geom_line(size = 1.5) + 
  coord_flip() +
  ...
  theme_bw() + 
  mytheme + 
  theme(legend.position = c(0.65, 0.25), legend.direction = "horizontal", plot.margin=unit(c(0.5,1,0.5,0.5), "lines"), axis.line.x = element_line(color='black'), axis.line.y = element_line(color='black')) 

plot

ggsave(plot, filename = "Figures/Figure 4.png",  width = 6, height = 6, units = "in", dpi = 600)
